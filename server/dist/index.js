...
app.get("/auth/discord/callback", rateLimit({ windowMs: 60_000, max: 30 }), async (req, res) => {
    try {
        ...
        if (!DISCORD_CLIENT_ID || !DISCORD_CLIENT_SECRET)
            return res.status(500).send("Discord env not set");
...
app.post("/api/party/create", rateLimit({ windowMs: 10_000, max: 20 }), (req, res) => {
    ...
    if (!parsed.success)
        return res.status(400).json({ error: "INVALID_BODY", details: parsed.error.flatten() });
    const { party, partyId, memberId } = STORE.createParty({ name: parsed.data.name, job: parsed.data.job, power: parsed.data.power }, parsed.data.title, parsed.data.passcode);
    broadcastParties();
...
app.post("/api/party/join", rateLimit({ windowMs: 10_000, max: 30 }), (req, res) => {
    ...
    if (!parsed.success)
        return res.status(400).json({ error: "INVALID_BODY", details: parsed.error.flatten() });
    const result = STORE.joinParty(parsed.data.partyId, { name: parsed.data.name, job: parsed.data.job, power: parsed.data.power }, parsed.data.passcode);
    if (!result.ok) {
...
app.patch("/api/party/:partyId/buffs", (req, res) => {
    ...
    if (!result.ok)
        return res.status(result.code === "FORBIDDEN" ? 403 : 404).json({ error: result.code });
